...
const Counters = () => {
    const [counter1, setCounter1] = useState(95);
    const [counter2, setCounter2] = useState(93);
    const [counter3, setCounter3] = useState(91);
    const [counter4, setCounter4] = useState(80);
    const { ref: ref1, inView: inView1 } = useInView({ triggerOnce: true });
    const { ref: ref2, inView: inView2 } = useInView({ triggerOnce: true });
    const { ref: ref3, inView: inView3 } = useInView({ triggerOnce: true });
    const { ref: ref4, inView: inView4 } = useInView({ triggerOnce: true });
     // Counter 1 - updates every 1 second when in view
  useEffect(() => {
    if (inView1) {
      const timer1 = setInterval(() => {
        setCounter1((prevCounter1) => {if(prevCounter1 >= 100){clearInterval(timer1) 
          return prevCounter1
        }else{ return prevCounter1 + 1}});
      }, 100);
    
      return () => clearInterval(timer1); // Cleanup on unmount
    }
  }, [inView1]);

  // Counter 2 - updates every 2 seconds when in view
  useEffect(() => {
    if (inView2) {
      const timer2 = setInterval(() => {
        setCounter2((prevCounter2) => {if(prevCounter2 >= 100){clearInterval(timer2)
           return prevCounter2 }else{ return prevCounter2 + 1}});
      }, 100);

      return () => clearInterval(timer2); // Cleanup on unmount
    }
  }, [inView2]);

  // Counter 3 - updates every 3 seconds when in view
  useEffect(() => {
    if (inView3) {
      const timer3 = setInterval(() => {
        setCounter3((prevCounter3) => {if(prevCounter3 >= 100){ clearInterval(timer3)
          return prevCounter3
         }else{ return prevCounter3 + 1}});
      }, 100);

      return () => clearInterval(timer3); // Cleanup on unmount
    }
  }, [inView3]);

  // Counter 4 - updates every 4 seconds when in view
  useEffect(() => {
    if (inView4) {
      const timer4 = setInterval(() => {
        setCounter4((prevCounter4) => {if(prevCounter4 >= 100){ 
          clearInterval(timer4)
        return prevCounter4
        }else{ return prevCounter4 + 1}});
      }, 100);

      return () => clearInterval(timer4); // Cleanup on unmount
    }
  }, [inView4]);
    return (
    <Container className='my-background1' fluid>
<div className="container ">
<div className="row py-5">
    <div className="col-xl-3 contofcounter" ref={ref1}>
        ...
        <h2 className="counter" id="count1">{counter1}</h2>
        <h4 className="countertext">Students Enrolled</h4>
    </div>
    <div className="col-xl-3 contofcounter " ref={ref2}>
        <div className="svgimg">
    ...
    <div className="col-xl-3 contofcounter" ref={ref4}>
        ...
        <h2 className="counter" id="count4">{counter4}</h2>
        <h4 className="countertext">Technical Courses</h4>
    </div>
</div>
</div>
    </Container>
